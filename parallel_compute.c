...
int main(int argc, char** argv) {
    ...
    start_time = MPI_Wtime(); // Zaman ölçümü başlat

    // MPI ile veriyi düğümlere dağıt
    MPI_Scatter(data, chunk_size, MPI_INT, sub_array, chunk_size, MPI_INT, MASTER, MPI_COMM_WORLD);

    // OpenMP ile her düğüm kendi verisini paralel olarak işler
    int local_sum = 0;
    int local_max = 0;
    ...
    #pragma omp parallel
    {
        ...
        // OpenMP threadlerinin hangi MPI süreci içinde çalıştığını göster
        printf("MPI Süreci %d - OpenMP Thread %d / %d çalışıyor\n", world_rank, thread_id, num_threads);
    }

    // MPI ile tüm düğümlerin verisini ana düğüme gönder
    int global_sum = 0;
    int global_max = 0;
    ...
    // Ana düğüm sonuçları ekrana yazdırır
    if (world_rank == MASTER) {
